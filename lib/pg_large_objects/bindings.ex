defmodule PgLargeObjects.Bindings do
  ...
  def create(repo, desired_oid)
      when is_atom(repo) and is_non_neg_integer(desired_oid) do
  ...
  def open(repo, oid, flags)
      when is_atom(repo) and is_pos_integer(oid) and is_non_neg_integer(flags) do
  ...
  def write(repo, fd, data) when is_atom(repo) and is_non_neg_integer(fd) and is_binary(data) do
  ...
  def read(repo, fd, length)
      when is_atom(repo) and is_non_neg_integer(fd) and is_non_neg_integer(length) do
  ...
  def lseek64(repo, fd, offset, whence)
      when is_atom(repo) and is_non_neg_integer(fd) and is_integer(offset) and
  ...
  def tell64(repo, fd) when is_atom(repo) and is_non_neg_integer(fd) do
